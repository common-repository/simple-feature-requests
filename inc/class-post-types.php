...
class SFR_Post_Types {
     ...
     * @var string
     */
    public static $sfr_slug = 'sfr_slug';

    public static $query_var = 'sfr_slug';
     ...
     * Run class.
     *
     * @param null|SFR_Core_Settings $settings
     */
    public static function run() {
        ...
        if ( is_admin() ) {
            require_once 'class-admin-filters.php';
            SFR_Admin_Filters::run();
            add_filter(
    ...
    public static function get_archive_page_id() {
        ...
        $archive_page_id = sfr_apply_filters( 'sfr_archive_page_id', $archive_page_id );
        return $archive_page_id;
    }
    ...
    public static function add( $options ) {
        ...
        $labels = array(
            ...
            'add_new'            => _x( 'Add New', 'simple-feature-requests' ),
            'add_new_item'       => _x( sprintf( 'Add New %s', $options['singular'] ), 'simple-feature-requests' ),
            'edit_item'          => _x( sprintf( 'Edit %s', $options['singular'] ), 'simple-feature-requests' ),
            'new_item'           => _x( sprintf( 'New %s', $options['singular'] ), 'simple-feature-requests' ),
            'view_item'          => _x( sprintf( 'View %s', $options['singular'] ), 'simple-feature-requests' ),
            'search_items'       => _x( sprintf( 'Search %s', $options['plural'] ), 'simple-feature-requests' ),
            'not_found'          => _x( sprintf( 'No %s found', strtolower( $options['plural'] ) ), 'simple-feature-requests' ),
            'not_found_in_trash' => _x( sprintf( 'No %s found in Trash', strtolower( $options['plural'] ) ), 'simple-feature-requests' ),
            'parent_item_colon'  => _x( sprintf( 'Parent %s:', $options['singular'] ), 'simple-feature-requests' ),
        ...
        if ( get_option( 'sfr_flush_rewrite_rules_flag' ) ) {
            flush_rewrite_rules();
     ...
     * Get current request.
     *
     * @return bool|SFR_Feature_Request
     */
    public static function get_current_request() {
        ...
        if ( empty( $slug ) ) {
            \SFR\Models\Log::error( 'get_current_request', 'Unable to a request slug in the url.' );
            return false;
        }
        $request_post = get_page_by_path( $slug, OBJECT, self::$key );
        if ( !$request_post ) {
            \SFR\Models\Log::error( 'get_current_request', 'Unable to locate a request with the slug of: ' . $slug );
            return false;
        }
        try {
            $request = new SFR_Feature_Request($request_post);
        } catch ( Exception $e ) {
            // Do nothing. We want to return null, which $request equals already.
            \SFR\Models\Log::error( 'get_current_request', 'There was a fatal error retrieving the request with the slug of: ' . $slug );
        }
        return $request;
    }
    ...
    public static function get_current_request_query() {
        ...
        $args = sfr_apply_filters( 'sfr_current_request_args', array(
            'post_type'        => self::$key,
            'posts_per_page'   => 1,
            'name'             => $slug,
    ...
    public static function get_page_type() {
        global $sfr_page_type;
        if ( !empty( $sfr_page_type ) ) {
            return (string) $sfr_page_type;
        }
    ...
    public static function admin_columns_content( $column, $post_id ) {
        ...
        $feature_request = new SFR_Feature_Request($post_id);
        if ( $column === 'status' ) {
            ...
            echo sfr_apply_filters( 'sfr_votes_count_column', $votes_count_text, $feature_request );
        }
    }
    ...
    public static function categories_meta_box() {
        ...
        echo Simple_Feature_Requests::get_pro_button();
    }
    ...
    public static function information_meta_box() {
        ...
        $meta = sfr_apply_filters( 'sfr_meta_information', array(
            'status' => array(
                'label'   => sprintf( '<label for="sfr_status">%s</label>', __( 'Status', 'simple-feature-requests' ) ),
		...
		<table class="sfr-meta-table widefat fixed">
			<?php 
        foreach ( $meta as $key => $meta_content ) {
            ?>
				<tr class="sfr-meta-table__row sfr-meta-table__row--<?php 
            echo esc_attr( $key );
            ?>">
					<th><?php 
    ...
    public static function get_meta_content_status_select( $feature_request ) {
        ...
        ob_start();
        ?>
		<select id="sfr_status" name="sfr_status">
			<?php 
    ...
    public static function get_inline_status_badge( $feature_request ) {
        $status = $feature_request->get_status();
        $status_label = sfr_get_status_label( $status );
    ...
    public static function get_posts_per_page() {
        $settings = SFR_Settings::get_settings();
        $posts_per_page = 10;
        // default
        if ( !empty( $settings['general_setup_ppp'] ) && is_numeric( $settings['general_setup_ppp'] ) ) {
            $posts_per_page = $settings['general_setup_ppp'];
        }
        return sfr_apply_filters( 'sfr_posts_per_page', $posts_per_page );
    }
    ...
    public static function get_single_title_tag() {
        ...
        if ( empty( $settings ) || empty( $settings['general_setup_single_title_tag'] ) ) {
            return sfr_apply_filters( 'sfr_single_title_tag', 'h1' );
        }
        return sfr_apply_filters( 'sfr_single_title_tag', $settings['general_setup_single_title_tag'] );
    }
    ...
    public static function get_archive_title_tag() {
        ...
        if ( empty( $settings ) || empty( $settings['general_setup_archive_title_tag'] ) ) {
            return sfr_apply_filters( 'sfr_archive_title_tag', 'h1' );
        }
        return sfr_apply_filters( 'sfr_archive_title_tag', $settings['general_setup_archive_title_tag'] );
    }
    ...
    public static function hide_entry_title() {
        ...
        if ( empty( $settings ) || empty( $settings['general_setup_hide_entry_title'] ) ) {
            return sfr_apply_filters( 'sfr_hide_entry_title', 0 );
        }
        return sfr_apply_filters( 'sfr_hide_entry_title', $settings['general_setup_hide_entry_title'] );
    }
    ...
    public static function flush_permalinks() {
        // Flush permalinks.
        if ( !get_option( 'sfr_flush_rewrite_rules_flag' ) ) {
            add_option( 'sfr_flush_rewrite_rules_flag', true );
        }
    }
    ...
    public static function admin_bar_menu_item() {
        ...
        $wp_admin_bar->add_menu( array(
            'id'    => $menu_id,
            'title' => sprintf( __( '%s', 'simple-feature-requests' ), $default_plural_name ),
            'href'  => '',
        ) );
        $default_single_name = sfr_apply_filters( 'sfr_single_request_name', 'Feature Request', true );
        if ( self::is_type( 'single' ) ) {
            $wp_admin_bar->add_menu( array(
                'parent' => $menu_id,
                'title'  => sprintf( __( 'Edit %s', 'simple-feature-requests' ), $default_single_name ),
                'id'     => 'sfr-edit',
                'href'   => get_edit_post_link(),
            ) );
        }
        $default_child_plural_name = sfr_apply_filters( 'sfr_child_plural_request_name', 'Feature Requests', true );
        $wp_admin_bar->add_menu( array(
            'parent' => $menu_id,
            'title'  => sprintf( __( 'Manage %s', 'simple-feature-requests' ), $default_child_plural_name ),
            'id'     => 'sfr-manage',
    ...
    public static function get_menu_title() {
        $title = __( sfr_apply_filters( 'sfr_plural_request_name', 'Requests', true ), 'simple-feature-requests' );
        $pending = SFR_Query::count_pending_requests();
        if ( empty( $pending ) ) {
            return $title;
        }
        return sprintf(
    ...
    public static function body_class( $classes ) {
        ...
        $classes[] = 'sfr';
        $classes[] = 'sfr--' . self::get_page_type();
        return $classes;
    }
    ...
    public static function add_pending_notice() {
        ...
        $feature_request = new SFR_Feature_Request($post->ID);
        $status = $feature_request->get_status();
        if ( 'pending' !== $status ) {
            return;
        }
        $status_description = sfr_get_status_description( $status );
    ...
    public static function attachments_meta_box( $post ) {
        ...
        ?>
		<div class="sfr-attachments-container">
			<ul class="sfr-attachments">
				<?php 
        foreach ( $attachment_ids as $attachment_id ) {
            ?>
				<li class="sfr-attachment" data-attachment_id="<?php 
            echo $attachment_id;
            ?>">
					<?php 
            echo wp_get_attachment_image( $attachment_id, 'thumbnail' );
            ?>
					<a hre="#" class="sfr-remove_attachment"></a>
				</li>
				<?php 
        }
        ?>
			</ul>
			<input type="hidden" name="sfr_attachments" value="<?php 
        echo implode( ',', $attachment_ids );
        ?>">

			<a href="#" class="sfr-add_attachments"><?php 
