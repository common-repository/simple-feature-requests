 ...
 * @return array
 */
function sfr_get_statuses( $excludes = array() ) {
	$statuses = array(
	...
	$statuses = sfr_apply_filters( 'sfr_statuses', $statuses );
 ...
 * @return bool|string
 */
function sfr_get_status_description( $status ) {
	$default_single_name = sfr_apply_filters( 'sfr_single_request_name', 'request', false );
	$descriptions = array(
	...
	$descriptions = sfr_apply_filters( 'sfr_status_descriptions', $descriptions );
...
function sfr_get_custom_statuses() {
	$statuses = wpsf_get_setting( 'sfr', 'general_setup', 'statuses' );

	return $statuses;
}

function sfr_get_custom_labels( $form = 'single' ) {

	$custom_label_single = wpsf_get_setting( 'sfr', 'general_labels', 'single_request_label');
	$custom_label_plural = wpsf_get_setting( 'sfr', 'general_labels', 'plural_request_label');
 ...
 * @return array
 */
function sfr_get_viewable_statuses() {
	...
	return sfr_apply_filters( 'sfr_viewable_statuses', $statuses );
}
 ...
 * @return array
 */
function sfr_get_term_options( $taxonomy, $hide_empty = true ) {
...
function sfr_get_status_slug( $title ) {
 ...
 * @return string
 */
function sfr_get_status_label( $status ) {
	$statuses = sfr_get_statuses();
...
function sfr_save_settings( $settings ) {
	if ( ! empty( $settings ) && is_array( $settings ) ) {
		update_option( 'sfr_settings', $settings );
		return true;
	} else {
		return false;
	}
...
function sfr_fix_post_statuses( $old_slug, $new_slug ) {
	if ( empty( $old_slug ) ) return false;
	if ( empty( $new_slug ) ) {
		$new_slug = sfr_apply_filters( 'sfr_default_blank_slug_status', 'pending' );
	}
	...
	if ( $requests = get_posts( $args ) ) {
		foreach ( $requests as $request ) {
			update_post_meta( $request->ID, 'sfr_status', $new_slug );
		}
		return true;
	}
}
 ...
 * @return mixed
 */
function sfr_get_status_colors( $status ) {
	$colors = array(
		'pending'      => array(
			'background' => '#FFE26C',
			'color'      => '#0F0F14',
	...
	$colors = sfr_apply_filters( 'sfr_status_colors', $colors );
 ...
 * @return string
 */
function sfr_get_default_post_status() {
	...
	return sfr_apply_filters( 'sfr_get_default_post_status', $default_status );
}
 ...
 * @return string
 */
function sfr_get_archive_url_with_filters( $excludes = array() ) {
	$url_parts = array(
		'base'  => SFR_Post_Types::get_archive_url(),
