...
class SFR_Query {
     ...
     * @var string
     */
    public static $top_requests_transient_name = 'sfr_top_requests';
    ...
    /**
     * Clears SFR caches when a request is updated.
     *
     * @param int $post_id
     * @param WP_POST $post
     * @return void
     */
    public static function clear_cache( $post_id, $post ) {
    ...
    public static function get_requests( $args = array() ) {
        $defaults = array(
            ...
            'sfr_query'      => true,
            'posts_per_page' => SFR_Post_Types::get_posts_per_page(),
            'paged'          => ( get_query_var( 'paged' ) ? get_query_var( 'paged' ) : 1 ),
        );
        $args = sfr_apply_filters( 'sfr_get_requests', wp_parse_args( $args, $defaults ) );
    ...
    public static function get_grouped_requests( $statuses = array(), $taxonomies = array() ) {
        ...
        $sfr_requests = self::get_requests( $query_args );
        if ( $sfr_requests->have_posts() ) {
            foreach ( $sfr_requests->posts as $sfr_request ) {
                $sfr_request = new SFR_Feature_Request($sfr_request);
                $status = $sfr_request->get_status();
                $grouped_requests[$status][] = $sfr_request;
            }
        }
    ...
    public static function is_query_modification_allowed( $query ) {
        ...
        if ( is_admin() || !$query->get( 'sfr_query' ) ) {
            return false;
        }
        return true;
    }
    ...
    public static function set_status_query( $query ) {
        ...
        $viewable_statuses = sfr_get_viewable_statuses();
    ...
    public static function get_top_requests() {
        ...
        $args = array(
            ...
            'meta_query'     => array(
                ...
                array(
                    'key'     => 'sfr_status',
                    'value'   => sfr_apply_filters( 'sfr_ignore_statuses_in_top_requests', array('completed', 'pending') ),
                    'compare' => 'NOT IN',
                ),
            ),
        ...
        foreach ( $query->posts as $request ) {
            $top_requests[] = new SFR_Feature_Request($request);
        }
