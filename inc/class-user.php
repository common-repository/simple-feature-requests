...
class SFR_User {
     ...
     * @var string
     */
    public static $votes_meta_key = '_sfr_votes';
    ...
    public static function handle_login() {
        if ( !isset( $_POST['sfr-login'] ) ) {
            return;
        }
        $notices = SFR_Notices::instance();
        $nonce = filter_input( INPUT_POST, 'sfr-login-nonce', FILTER_SANITIZE_SPECIAL_CHARS );
        $username = trim( filter_input( INPUT_POST, 'sfr-login-username', FILTER_SANITIZE_SPECIAL_CHARS ) );
        $email = trim( filter_input( INPUT_POST, 'sfr-login-email', FILTER_SANITIZE_SPECIAL_CHARS ) );
        $password = trim( filter_input( INPUT_POST, 'sfr-login-password', FILTER_SANITIZE_SPECIAL_CHARS ) );
        $repeat_password = trim( filter_input( INPUT_POST, 'sfr-login-repeat-password', FILTER_SANITIZE_SPECIAL_CHARS ) );
        $user_type = trim( filter_input( INPUT_POST, 'sfr-login-user-type', FILTER_SANITIZE_SPECIAL_CHARS ) );
        if ( !wp_verify_nonce( $nonce, 'sfr-login' ) ) {
        ...
        if ( $user_type === 'register' ) {
            ...
            SFR_User::login( $email, $password );
        }
        if ( $notices->has_notices() ) {
            return;
        }
        wp_safe_redirect( SFR_Post_Types::get_archive_url(), 302 );
        exit;
    }
    ...
    public function get_votes_count() {
        ...
        $reimburse_statuses = ( !empty( $settings['votes_limits_reimburse_votes'] ) && is_array( $settings['votes_limits_reimburse_votes'] ) ? $settings['votes_limits_reimburse_votes'] : [] );
        foreach ( $votes as $request_id => $vote ) {
            $request = new SFR_Feature_Request($request_id);
