 ...
 * @since 4.6.11
 */
class SFR_Compat_Elementor {
	/**
	 * Init.
	 */
	public static function run() {
		if ( ! SFR_Core_Helpers::is_plugin_active( 'elementor/elementor.php' ) ) {
			return;
		}
	...
	public static function elementor_edit_mode( $value, $object_id, $meta_key, $single ) {
		...
		if ( SFR_Post_Types::$key !== get_post_type( $object_id ) ) {
			return $value;
		}

		$archive_id = SFR_Post_Types::get_archive_page_id();
	...
	public static function elementor_document_id( $post_id ) {
		...
		if ( ! SFR_Post_Types::is_type( 'single' ) || $post_type !== SFR_Post_Types::$key ) {
			return $post_id;
		}
	...
	public static function enqueue_styles() {
		if ( Plugin::$instance->preview->is_preview_mode() || ! SFR_Post_Types::is_type( 'single' ) ) {
			return;
		}
