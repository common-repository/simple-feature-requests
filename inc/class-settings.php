...
class SFR_Settings {
    ...
    public static function run() {
        ...
        add_filter( 'wpsf_menu_icon_url_sfr', array(__CLASS__, 'menu_icon') );
        add_filter( 'wpsf_menu_position_sfr', array(__CLASS__, 'menu_position') );
        add_filter( 'wpsf_register_settings_sfr', array(__CLASS__, 'add_settings'), 10 );
        add_filter( 'wpsf_register_settings_sfr', array(__CLASS__, 'add_free_settings'), 20 );
        add_action( 'update_option_sfr_settings', array(__CLASS__, 'after_save_settings'), 10 );
        add_action( 'add_option_sfr_settings', array(__CLASS__, 'after_save_settings'), 10 );
    ...
    public static function add_menu_items() {
        global $submenu;
        $settings_framework = SFR_Core_Settings::$settings_framework;
        add_submenu_page(
            'sfr-settings',
            __( 'Add New', 'simple-feature-requests' ),
            __( 'Add New', 'simple-feature-requests' ),
            'manage_options',
        ...
        $submenu['sfr-settings'][] = array(__( 'Documentation', 'simple-feature-requests' ), 'manage_options', 'https://docs.simplefeaturerequests.com/?utm_source=SFR&utm_medium=insideplugin');
        $submenu['sfr-settings'][] = array(__( 'Support', 'simple-feature-requests' ), 'manage_options', 'https://simplefeaturerequests.com/contact/?utm_source=SFR&utm_medium=insideplugin');
    }
    ...
    public static function support_link() {
        return sprintf( '<a href="%s" class="button button-secondary" target="_blank">%s</a>', 'http://www.simplefeaturerequests.com/contact/?utm_source=SFR&utm_medium=insideplugin', __( 'Submit Ticket', 'simple-feature-requests' ) );
    }
    ...
    public static function documentation_link() {
        return sprintf( '<a href="%s" class="button button-secondary" target="_blank">%s</a>', 'https://docs.simplefeaturerequests.com/?utm_source=SFR&utm_medium=insideplugin', __( 'Read Documentation', 'simple-feature-requests' ) );
    }
    ...
    public static function add_settings( $settings ) {
        ...
        $settings['sections']['general_setup'] = array(
            ...
            'fields'              => array(
                'archive_page_id' => array(
                    ...
                    'type'     => 'select',
                    'choices'  => SFR_Settings::get_page_options(),
                    'default'  => '',
                ),
            ),
        );
        ...
        $settings['sections']['general_attachments'] = array(
            'tab_id'              => 'general',
            'section_id'          => 'attachments',
            'section_title'       => __( 'Attachments', 'simple-feature-requqests' ),
            'section_description' => '',
            'section_order'       => 30,
            'fields'              => array(array(
                'id'       => 'allow_attachments',
                'title'    => __( 'Allow Attachments', 'simple-feature-requests' ),
                'subtitle' => __( 'Allow users to upload attachments with their feature request', 'simple-feature-requests' ),
                'type'     => 'custom',
                'default'  => Simple_Feature_Requests::get_pro_button(),
            )),
        );
        $settings['sections']['general_filters'] = array(
            'tab_id'              => 'general',
            'section_id'          => 'filters',
            'section_title'       => __( 'Filters', 'simple-feature-requqests' ),
            'section_description' => '',
            'section_order'       => 30,
            'fields'              => array(array(
                'id'       => 'show_filters',
                'title'    => __( 'Filters', 'simple-feature-requests' ),
                'subtitle' => __( 'Add filters to search results for: Latest, Top, My Requests, Status, Board.', 'simple-feature-requests' ),
                'type'     => 'custom',
                'default'  => Simple_Feature_Requests::get_pro_button(),
            )),
        );
        ...
        $settings['sections']['votes_general'] = array(
            ...
            'fields'              => array(array(
                ...
                'title'    => __( 'Allow Guest Voting', 'simple-feature-requests' ),
                'subtitle' => __( 'Can votes be added without a logged in user account?', 'simple-feature-requests' ),
                'type'     => 'custom',
                'default'  => Simple_Feature_Requests::get_pro_button(),
            )),
        );
    ...
    public static function get_page_options() {
        $return = array(__( 'Select a page...', 'simple-feature-requests' ));
    ...
    public static function populate_slug_tracker( $statuses ) {
        ...
        foreach ( $statuses as $key => $status ) {
            $tracker[$status['row_id']] = sfr_get_status_slug( $status['status_title'] );
        }
        $tracker = array_filter( $tracker );
        update_option( 'sfr_status_slug_tracker', $tracker );
        return $tracker;
    }
    ...
    public static function compare_slug_tracker( $statuses, $tracker ) {
        ...
        foreach ( $statuses as $status ) {
            $existing[$status['row_id']] = sfr_get_status_slug( $status['status_title'] );
        }
        $leftovers = $tracker;
        foreach ( $existing as $row_id => $slug ) {
            ...
            if ( !array_key_exists( $row_id, $tracker ) ) {
                ...
                if ( $slug != $tracker[$row_id] ) {
                    ...
                    sfr_fix_post_statuses( $tracker[$row_id], $slug );
                    $tracker[$row_id] = $slug;
                    $update_tracker = true;
                }
            }
        ...
        if ( !empty( $leftovers ) ) {
            foreach ( $leftovers as $leftover_row_id => $left_over_slug ) {
                sfr_fix_post_statuses( $left_over_slug, sfr_apply_filters( 'sfr_default_missing_status', 'pending' ) );
    ...
    public static function after_save_settings() {
        $slug_tracker = get_option( 'sfr_status_slug_tracker', array() );
        ...
        SFR_Post_Types::flush_permalinks();
    }
    ...
    public static function archive_page_notice() {
        $archive_url = SFR_Post_Types::get_archive_url();
        ...
        ?></strong>
				<?php 
        printf( __( 'You need to <a href="%s">set an archive page</a> for Simple Feature Requests, under the "General" tab.', 'simple-feature-requests' ), admin_url( 'admin.php?page=sfr-settings' ) );
        ?>
			</p>
		</div>
		<?php 
    ...
    /**
     * Retrieves an individual setting value. Returns null if setting not set.
     *
     * @param string Name of the setting to retrieve.
     * @return mixed
     */
