...
class SFR_AJAX {
	...
	public static function add_ajax_events() {
		...
		foreach ( $ajax_events as $ajax_event => $nopriv ) {
			add_action( 'wp_ajax_sfr_' . $ajax_event, array( __CLASS__, $ajax_event ) );

			if ( $nopriv ) {
				add_action( 'wp_ajax_nopriv_sfr_' . $ajax_event, array( __CLASS__, $ajax_event ) );
			}
		}
	}
	...
	public static function update_vote_count() {
		...
		$check_nonce = check_ajax_referer( 'sfr-nonce', 'nonce', false );
	...
	public static function search_feature_requests() {
		...
		$args = sfr_apply_filters( 'sfr_search_feature_requests_args', array(
		...
		$sfr_requests        = new WP_Query( $args );
		$sfr_requests->paged = $paged;

		$response['count'] = $sfr_requests->found_posts;

		ob_start();

		if ( $sfr_requests->have_posts() ) {
			$response['success'] = true;
			while ( $sfr_requests->have_posts() ) : $sfr_requests->the_post();
				sfr_do_action( 'sfr_loop' );
			endwhile;
		} else {
			sfr_do_action( 'sfr_no_requests_found' );
		}

		$response['html'] = ob_get_clean();

		ob_start();
		SFR_Template_Hooks::include_template( 'loop/pagination', array(
			'add_args' => array(
		...
		$response = sfr_apply_filters( 'sfr_search_feature_requests_response', $response );
	...
	public static function set_feature_request_attachments(){
		...
		$nonce = check_ajax_referer( 'sfr-attachment-nonce', 'nonce', false );
