...
class SFR_Factory {
	...
	public static function run() {
		...
		add_action( 'sfr_status_updated', array( __CLASS__, 'status_updated' ), 10, 2 );
	}
	...
	public static function save_request( $post_id, $post, $update ) {
		...
		$status = filter_input( INPUT_POST, 'sfr_status' );
		$votes  = filter_input( INPUT_POST, 'sfr_votes_count', FILTER_SANITIZE_NUMBER_INT );
	...
	public static function create( $args = array() ) {
		...
		$args = sfr_apply_filters( 'sfr_create_post_args', wp_parse_args( $args, $defaults ) );
		...
		$reserved  = sfr_apply_filters( 'sfr_reserved_slugs', array( 'page' ) );
		$page_name = sfr_apply_filters( 'sfr_request_slug', sanitize_title( $args['title'] ), $args );
		...
		$default_status  = sfr_get_default_post_status();
		$feature_request = new SFR_Feature_Request( $inserted_id );
		...
		if ( $args['board'] !== false ) {
			wp_set_object_terms( $inserted_id, $args['board'], SFR_BOARD_TAXONOMY_NAME );
		}

		sfr_do_action( 'sfr_post_created', $inserted_id );
	...
	public static function merge( $primary_request_id, $requests = array(), $options = array() ) {
		...
		$primary_request = new SFR_Feature_Request( $primary_request_id );
		...
		foreach ( $requests as $request_id ) {
			...
			sfr_do_action( 'sfr_request_merged', $primary_request, $request );
		}
	}
}